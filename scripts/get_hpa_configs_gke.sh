...
do 
  _TARGET_PERCENTAGE=$(kubectl get hpa $_HPA --no-headers | awk '{print $3}' | cut -d '/' -f 2 | cut -d '%' -f 1)
  _MIN_REPLICAS=$(kubectl get hpa $_HPA --no-headers | awk '{print $4}')
  _MAX_REPLICAS=$(kubectl get hpa $_HPA --no-headers | awk '{print $5}') 

...
class GaleraAVS68Util:
    ...
    def initialise(self,config_dir):
        ...
        with open('../config/galera-avs68-configs.json', 'r') as f:
    ...
    def run(self):
        ...
        try:
            ...
            stdin, stdout, stderr = ssh.exec_command("/product/galera-backup/db_backup.sh %s %s %s >> /product/galera-backup/galera_backup_%s.log" %(dbuser, dbPassword, backupBucket, self.timestamp), timeout=120000)
            self.log.debug(stdout.readlines())
            ssh.close()
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())

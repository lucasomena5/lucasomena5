...
resource "aws_subnet" "public_subnet" {
  ...
  ipv6_cidr_block                 = count.index == 1 ? cidrsubnet(aws_vpc.vpc.ipv6_cidr_block, 8, sum([count.index, 10])) : cidrsubnet(aws_vpc.vpc.ipv6_cidr_block, 8, sum([count.index, 30]))
...
resource "aws_subnet" "private_subnet" {
  ...
  ipv6_cidr_block                 = count.index == 1 ? cidrsubnet(aws_vpc.vpc.ipv6_cidr_block, 8, sum([count.index, 60])) : cidrsubnet(aws_vpc.vpc.ipv6_cidr_block, 8, sum([count.index, 80]))
  assign_ipv6_address_on_creation = true
  tags = {
